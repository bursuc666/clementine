...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function get_logo_image() {
			$image_url = ETHEME_BASE_URI.'theme/assets/images/logo-fixed.png';
		...
		public function init_globals() {
			...
			$this->page_slug       = apply_filters( $this->theme_name . '_theme_setup_wizard_page_slug', 'xstore-setup' );
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'xstore' ), esc_html__( 'Setup Wizard', 'xstore' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'xstore' ), esc_html__( 'Setup Wizard', 'xstore' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'xstore' ),
			...
			$this->steps['default_content'] = array(
				'name'    => esc_html__( 'Content', 'xstore' ),
				'view'    => array( $this, 'envato_setup_default_content' ),
				'handler' => '',
			);
			if ( class_exists( 'TGM_Plugin_Activation' ) && isset( $GLOBALS['tgmpa'] ) ) {
				$this->steps['default_plugins'] = array(
					'name'    => esc_html__( 'Plugins', 'xstore' ),
			...
			$this->steps['help_support']    = array(
				'name'    => esc_html__( 'Support', 'xstore' ),
				'view'    => array( $this, 'envato_setup_help_support' ),
				'handler' => '',
			);
			$this->steps['next_steps']      = array(
				'name'    => esc_html__( 'Ready!', 'xstore' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'xstore' ),
			) );
		...
		public function setup_wizard_header() {
			...
			<head>
				...
				echo '<t';
				echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'xstore' ) . '</ti' . 'tle>'; ?>
			...
			<?php if ( 'next_steps' === $this->step ) : ?>
				<a class="wc-return-to-dashboard"
				   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'xstore' ); ?></a>
			<?php endif; ?>
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>big-eight.png" alt="eight theme" class="elogo">
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'xstore' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ', 'xstore' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'xstore' ); ?></a>
					</li>
			<!-- 		<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Reset font style and colors', 'xstore' ); ?>" name="save_step"/>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</form>
					</li> -->
				</ul>
				<p style="border:2px solid #ff4242; color:#c33434; padding:10px;">Important! If you need to update XStore theme, just Cancel the setup wizard. 
...
Go to Theme Options and Activate theme with your Purchase code.</p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'xstore' ); ?></a>
				</p>
				<?php
			} else {
				?>
				...
				<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>big-eight.png" alt="eight theme" class="elogo">
				<h1><?php printf( esc_html__( 'Welcome to XStore Setup Wizard', 'xstore' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing our XStore template.', 'xstore' ), wp_get_theme() ); ?></p>
				<p><?php printf( esc_html__( 'This setup wizard will help you to refresh and configure your website with a new layout. You will have Child Theme, Content, and Plugins installed in 5-10 minutes (depending on your server configuration). ', 'xstore' ), wp_get_theme() ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you do not want to go through the wizard, you can skip, and get back to WordPress dashboard. Come back any time to continue!', 'xstore' ); ?></p>
				<p style="border:2px solid #ff4242; color:#c33434; padding:10px;">Important! If you need to update XStore theme, just Cancel the setup wizard. 

Go to Theme Options and Activate theme with your Purchase code.</p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button button-active button-next"><?php esc_html_e( 'Let\'s Go!', 'xstore' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="et-button"><?php esc_html_e( 'Not right now', 'xstore' ); ?></a>
				</p>
				<?php
			}
		}
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'xstore' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'xstore' ); ?></h1>
			<form method="post" class="plugins-form" data-version="<?php echo esc_attr( $version_import ); ?>">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website requires some additional plugins.  The following plugins will be installed:', 'xstore' ); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'xstore' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'Please, note that every external plugin can affect your website loading speed. You can add and remove plugins later on from within WordPress.', 'xstore' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   ...
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'xstore' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button"><?php esc_html_e( 'Skip this step', 'xstore' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function envato_setup_default_content() {
			...
			?>
			<h1><?php esc_html_e( 'Default Content', 'xstore' ); ?></h1>
			<form method="post">
...
Once imported, this content can be managed from the WordPress dashboard.  </p>
	            ...
	            <div class="import-demos-wrapper">
	                ...
	                <div class="import-demos">
	                    <?php $i=0; foreach ($demos as $key => $version): $i++; ?>
	                        <div class="version-preview <?php if( $i==1 ) echo 'active-version '; echo ( in_array( $key, $versions_imported ) ) ? 'version-imported' : 'not-imported'; ?> version-preview-<?php echo esc_attr( $key ); ?>" data-version="<?php echo esc_attr( $key ); ?>">
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button button-active button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'xstore' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button"><?php esc_html_e( 'Skip this step', 'xstore' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo &amp; Design', 'xstore' ); ?></h1>
			<form method="post">
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'xstore' ); ?></a>
						</td>
					</tr>
				</table>
				...
				if ( ! $this->get_default_theme_style() || count( $demo_styles ) <= 1 ) {
					...
					<p><?php esc_html_e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.', 'xstore' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'xstore' ); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'xstore' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function envato_setup_updates() {
			...
			<?php if ( etheme_is_activated() ): ?>
				<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>success-icon-alt.png" alt="eight theme" class="elogo">
				...
				<?php if( get_filesystem_method() === 'direct' ) : ?>
					...
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="et-button button-active"><?php esc_html_e( 'Continue', 'xstore' ); ?></a>
						<?php wp_nonce_field( 'envato-setup' ); ?>
					</p>
				...
				<?php else: ?>
					...
					<div class="support-includes">
						<div>
							<p><?php esc_html_e( 'More Resources', 'xstore' ); ?></p>
							<ul>
								<li class="documentation"><a href="http://8theme.com/demo/docs/xstore/" target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'xstore' ); ?></a></li>
								<li class="howto"><a href="https://wordpress.org/support/"target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'xstore' ); ?></a></li>
								<li class="rating"><a href="http://themeforest.net/downloads"target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'xstore' ); ?></a></li>
								<li class="support"><a href="https://www.8theme.com/forums/"target="_blank"><?php esc_html_e( 'Get Help and Support', 'xstore' ); ?></a></li>
							</ul>
						</div>
					...
					<p class="envato-setup-actions step">
						<a class="et-button button-active" href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'xstore' ); ?></a>
						<a class="et-button" href="https://themeforest.net/user/8theme" target="_blank"><?php esc_html_e( 'Follow @8theme on ThemeForest', 'xstore' ); ?></a>
					</p>

				<?php endif;?>

			<?php else: ?>
				<h1><?php _e('Activate', 'xstore'); ?></h1>
				<form method="post">
	                <p>
	                    <label for="purchase-code"><?php _e('Enter Purchase code', 'xstore'); ?></label>
	                    <input type="text" name="purchase-code" placeholder="Example: f20b1cdd-ee2a-1c32-a146-66eafea81761" id="purchase-code" />
	                </p>
	                <p>
	                        <input class="et-button" name="xstore-purchase-code" type="submit" value="<?php esc_attr_e( 'Activate theme', 'xstore' ); ?>" />
		...
		?>
			<h1>Setup Xstore Child Theme (Optional)</h1>
			...
			<?php } else { ?>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="et-button button-active"><?php _e( 'Continue', 'xstore' ); ?></a>
			</p>
			<?php } ?>
			<?php
		}
		...
		private function _make_child_theme( $new_theme_title ) {
				...
				if ( file_exists( $new_theme_path ) ) {
					...
					$plugin_folder = get_template_directory().'/framework/thirdparty/envato_setup/child-theme/';
		...
		public function envato_setup_help_support() {
			...
			<ul class="support-blocks">
				<li>
					<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>chat-icon.png"><br>
					Live Chat 24/7<br> <a href="http://8theme.com/demo/xstore/previews/" target="_blank">check</a></li>
				<li>
					<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>support-icon.png"><br>
					Support Forum<br> <a href="https://www.8theme.com/forums/" target="_blank">check</a></li>
				<li>
					<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>icon-envato.png"><br>
		...
		public function envato_setup_ready() {
			...
			<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>success-icon-alt.png" alt="eight theme" class="elogo">
			<h1><?php esc_html_e( 'Your Website is Ready!', 'xstore' ); ?></h1>

			<p>Congratulations! XStore theme has been activated on your website and ready to use. Login to your WordPress dashboard to make all the necessary changes and upload your content. </p>
			...
			<div class="support-includes">
				<div>
					<p><?php esc_html_e( 'More Resources', 'xstore' ); ?></p>
					<ul>
						<li class="documentation"><a href="http://8theme.com/demo/docs/xstore/"
						                             target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'xstore' ); ?></a>
						</li>
						<li class="howto"><a href="https://wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'xstore' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'xstore' ); ?></a></li>
						<li class="support"><a href="https://www.8theme.com/forums/"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'xstore' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<p>Come back and leave us <span>5-star rating</span></p>
					<br>
					<iframe width="560" height="315" style="max-width:100%; height:auto;" src="https://www.youtube.com/embed/nMgqUBcOhXA?list=PLMqMSqDgPNmCCyem_z9l2ZJ1owQUaFCE3" frameborder="0" allowfullscreen></iframe>
				</div>
			</div>
			<p class="envato-setup-actions step">
				<a class="et-button button-active" href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'xstore' ); ?></a>
