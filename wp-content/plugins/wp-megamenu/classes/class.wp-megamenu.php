    ...
    class wp_megamenu extends Walker_Nav_Menu{
        ...
        function start_lvl(&$output, $depth = 0, $args = array()) {
            ...
            if ($this->container_type === 'wpmm-strees-row-and-content'){
                $is_row_content_strees_extra_div = " <div 
class='wpmm-row-content-strees-extra'></div> ";
            }
...
function overrite_functions_wp_megamenu($args){
    ...
    if (count($auto_intergration_menu)){
        ...
        if (! empty($brand_logo)){
            ...
            $brand_logo_wrap = "<div class='wpmm_brand_logo_wrap'><a href='{$home_url}'> <img src='{$brand_logo}' {$logo_width} {$logo_height}
 /> </a> </div>";
        }        
        ...
        return $argunets = array(
            ...
            'container_class'   => "wp-megamenu-wrap wpmm-mobile-menu {$sticky_class} {$menu_layout_class} {$menu_custom_class} ",
...
function wp_megamenu_add_menu_settings_wrap_admin_footer() {
    ...
    if (property_exists($current_screen,'base')){
        if ($current_screen->base === 'nav-menus'){
            $html = '<div id="wpmmSettingOverlay" style="display: none;"></div><div class="wp-megamenu-item-settins-wrap" style="display: 
none;">';
